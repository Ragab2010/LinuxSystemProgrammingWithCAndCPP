...
void forkToInvokeVim(const char *);
void readFromCin(std::istream &, int);
int getPageLen();

void do_more(std::istream &, char *, int); // Function prototype

int main(int argc, char *argv[])
{
    int PAGELEN = getPageLen(); // Get the dynamic value of PAGELEN
    ...
    if (argc == 1)
    {
        ...
        do_more(openFile, tempFileName, PAGELEN);
        remove(tempFileName);
    }
    else
    {
...
void do_more(std::istream &file, char *arg, int PAGELEN)
{
    std::string line;
    int num_of_lines = 0;
    int total_lines = 0;
    ...
    while (std::getline(file, line))
    {
        ...
        num_of_lines++;
        double pre = ((double)num_of_lines / total_lines) * 100;
...
void forkToInvokeVim(const char *argv)
{
    auto id = fork();
    if (id == 0)
    {
        // child
        char *const cmd[] = {"vim", const_cast<char *>(argv), NULL};
...
int getPageLen()
{
    ...
    return w.ws_row - 1; // Subtract 1 for status line
}
